	...
	protected function _getFields(&$component_object,&$fieldset = null)
	{
		...
		foreach ($fields as $field)
		{
			...
			{		
				...
				if ($type == 'calendar' AND empty($field->field_filter))
				{
					$field->field_filter = 'fixed';
				}
				...
				switch ($type)
				{
					...
					case 'calendar':
						...
						$field_site_value = 'echo ($this->item->'.JString::trim($field->code_name).' != \'\' AND $this->item->'.JString::trim($field->code_name).' != \'0000-00-00 00:00:00\') ? JHtml::date($this->item->'.JString::trim($field->code_name).', \''.$field->format.'\', '.$field_filter.') : $empty;';
						$registry_entry_site_value = 'echo ($field_array[\''.JString::trim($field->code_name).'\'] != \'\' AND $field_array[\''.JString::trim($field->code_name).'\'] != \'0000-00-00 00:00:00\') ? JHtml::date($field_array[\''.JString::trim($field->code_name).'\'], \''.$field->format.'\', '.$field_filter.')  : $empty;';
