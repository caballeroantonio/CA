		...
		<table class="<?php echo version_compare(JVERSION, '3.0', 'ge') ? 'table table-striped':'adminlist'; ?>" id="field-list">
			...
			<?php foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'fields.', $can_change); ?>
							<?php
								if ($archived) :
									JHtml::_('actionsdropdown.unarchive', 'cb' . $i, 'fields');
								else :
									JHtml::_('actionsdropdown.archive', 'cb' . $i, 'fields');
								endif;
								if ($trashed) :
									JHtml::_('actionsdropdown.untrash', 'cb' . $i, 'fields');
								else :
									JHtml::_('actionsdropdown.trash', 'cb' . $i, 'fields');
								endif;
