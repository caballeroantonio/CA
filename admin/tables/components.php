...
class ComponentArchitectTableComponents extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_componentarchitect.component'));
	}
	...
	public function store($update_nulls = false)
	{
		...
		if (count($files) > 0 AND isset($files['categories_icon_16px']) AND $files['categories_icon_16px']['name'] != '')
		{
			$file = $files['categories_icon_16px'];		
			
			// Add parameters to the saveUpload call to specify the output file name and the width and height for image file uploads
			// e.g. saveUpload($file,'categories_icon_16px.png', 100, 0)
			//[%%START_CUSTOM_CODE%%]
			//$result = ComponentArchitectHelper::saveUpload($file, 'Categories Icon 16px');
			$result = ComponentArchitectHelper::saveUpload($file, 'Categories Icon 16px', '', 16, 16);
			//[%%END_CUSTOM_CODE%%]
			
			if ($result === true)
			{
				$this->categories_icon_16px = $file['name'];
			}
			else
			{
				if ($result !== false)
		...
		if (count($files) > 0 AND isset($files['categories_icon_48px']) AND $files['categories_icon_48px']['name'] != '')
		{
			$file = $files['categories_icon_48px'];		
			
			// Add parameters to the saveUpload call to specify the output file name and the width and height for image file uploads
			// e.g. saveUpload($file,'categories_icon_48px.png', 100, 0)
			//[%%START_CUSTOM_CODE%%]
			//$result = ComponentArchitectHelper::saveUpload($file, 'Categories Icon 48px');
			$result = ComponentArchitectHelper::saveUpload($file, 'Categories Icon 48px', '', 48, 48);
			//[%%END_CUSTOM_CODE%%]
			
			if ($result === true)
			{
				$this->categories_icon_48px = $file['name'];
			}
			else
			{
				if ($result !== false)
