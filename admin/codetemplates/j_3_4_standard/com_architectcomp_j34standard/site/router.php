 ...
 *
 * @version			$Id: router.php 590 2016-01-13 16:03:06Z BrianWade $
...
class [%%ArchitectComp%%]Router extends JComponentRouterBase
{
	...
	public function build(&$query)
	{
		...
		// Check again
		if ($menu_item_given AND isset($menu_item) AND $menu_item->component != '[%%com_architectcomp%%]')
		{
			$menu_item_given = false;
			unset($query['Itemid']);
		}
		...
		if (isset($query['view']))
		{
			...
			{
				if (empty($query['Itemid']) || empty($menu_item) || $menu_item->component != '[%%com_architectcomp%%]')
				{
					$segments[] = $query['view'];
					unset($query['view']);			
				}
	...
	public function parse(&$segments)
	{
		...
		foreach($segments as $segment)
		{
			...
			{
				[%%FOREACH COMPONENT_OBJECT%%]
				if ($item->query['view'] == '[%%compobject%%]' OR $view == '[%%compobject%%]')
				{
					if($advanced)
					{
						$db = JFactory::getDbo();
				...
				if ($item->query['view'] == '[%%compobjectplural%%]'OR $view == '[%%compobjectplural%%]')
				{
					$vars['view'] = $view;
				}				
				[%%ENDFOR COMPONENT_OBJECT%%]	
